...
class ReliabilityAnalysisTool:
    ...
    def run(self):
        ...
        # Pass limits for Normalized Slope calculation
        screening_df = trend_predictor.calculate_all_trends(limits_df=self.loader.limits_df)
        
        # Visualize Screening Summary
        # Update visualizer to plot Norm_Slope if available
        ...
        for target_col in top_items:
            ...
            dist_img = self.visualizer.plot_distribution_comparison(
                self.loader.qim_df[target_col], 
                self.loader.asrp_df[target_col], 
                self.loader.overhaul_df[target_col],
                target_col,
                limits=self.loader.limits_df
            )
            overview_img = self.visualizer.plot_data_overview(
                self.loader.df, target_col, limits=self.loader.limits_df
            )
            ...
            trend_img = self.visualizer.plot_multi_model_prediction(
                self.loader.df, target_col, future_months, all_predictions, limits=self.loader.limits_df
            )
            
            # Model Performance (RMSE)
            rmse_metrics = trend_predictor.model_metrics.get(target_col, {})
            perf_img = self.visualizer.plot_model_performance(rmse_metrics, target_col)
            ...
            item_report = self.generate_item_section(target_col, stats, dist_img, overview_img, trend_img, perf_img, pairs_img, rmse_metrics)
    ...
    def generate_item_section(self, col, stats, dist_img, overview_img, trend_img, perf_img, pairs_img, rmse_metrics):
        # Format RMSE table
        sorted_rmse = sorted(rmse_metrics.items(), key=lambda x: x[1])
        rmse_rows = [f"| {model} | {rmse:.4f} |" for model, rmse in sorted_rmse]
        rmse_table = "\n".join(rmse_rows)
...
*   **예측 범위**: 20년 (240개월)
*   **신뢰 구간**: 90%
*   **최적 모델**: **{sorted_rmse[0][0]}** (RMSE: {sorted_rmse[0][1]:.4f})
...
if __name__ == "__main__":
    # v1 Data Directory (Scenario)
    data_dir = "c:/Antigravity/missile_reliability_proto_v1/v1_code/scenario_data"
    tool = ReliabilityAnalysisTool(data_dir, output_dir="scenario_output")
    tool.run()
