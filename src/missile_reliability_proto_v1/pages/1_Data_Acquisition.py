        ...
        if st.button("PDF 변환 실행 (Convert)", key="btn_pdf"):
            with st.spinner("PDF 분석 및 변환 중... (EasyOCR 엔진 사용)"):
                try:
                    ...
                    limits_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'upper_lower_limit.csv'))
...
with tab2:
    ...
    # Default path setup
    default_csv_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'core', 'csv2csv'))
